...
mod tests {
    ...
    fn available_with_flags() {
        ...
        assert_eq!(config.heartbeat_seconds(90), 30);
        assert_eq!(config.heartbeat_seconds(30), 10);
        assert_eq!(config.heartbeat_seconds(300), 90);
    ...
    fn invalid_limits() {
        ...
        let mut c = caps();
        c.limits.recommended_heartbeat_seconds = 10;
        assert_eq!(negotiate(&c, "1.7.3"), Negotiation::InvalidCapabilities);

        let mut c = caps();
        c.limits.presence_lease_min_seconds = 400;
        assert_eq!(negotiate(&c, "1.7.3"), Negotiation::InvalidCapabilities);
    }
